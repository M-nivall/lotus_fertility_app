...
import static com.example.Varsani.utils.Urls.URL_REQUESTED_EQUIPMENTS;
...
import com.example.Varsani.Staff.Store_mrg.Adapter.AdapterEquipments;
...
public class RequestedEquipments extends AppCompatActivity {
    private RecyclerView recyclerView;
    private ProgressBar progressBar;
    private List<RequestModel> list;
    private AdapterEquipments adapter;
    private Button btn_next;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_requested_equipments);

        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle("Requested Equipments");
    ...
    public void equipments(){
        StringRequest stringRequest=new StringRequest(Request.Method.POST, URL_REQUESTED_EQUIPMENTS,
                        ...
                        try {
                            ...
                            if(status.equals("1")){
                                ...
                                for(int i=0; i <jsonArray.length();i++){
                                    ...
                                    String item_type=jsn.getString("amount");

                                    RequestModel rq=new RequestModel(requestID,  name,  phoneNo,
                                            items,  requestDate,  requestStatus, amount, quantity,item_type);
                                    list.add(rq);
                                }
                                adapter=new AdapterEquipments(getApplicationContext(),list);
